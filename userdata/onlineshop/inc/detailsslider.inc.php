...
if($this->phpactive())
{
	if($this->get_setting('cbUsePhpUsercomments_Checked') == 'True')
	{
		...
		if($commcount > 0)
		{
            ...
            $reviews = str_replace('{GSSE_CLASS_DSREVIEWS}','',$reviews);
		}
		else
		{
			$avgrating_html = '';
			$review_items = $this->get_lngtext('LangTagNoRatings');
            $reviews = str_replace('{GSSE_CLASS_DSREVIEWS}','display: none;',$reviews);
		}
...
if($all_items == ''){
    ...
    $detailsslider = str_replace('{GSSE_CLASS_MANUINFOS}','',$detailsslider);
}
/*Downloads*/
$all_items = '';
...
if($dlmax > 0)
{
	for($ad = 0; $ad < $dlmax; $ad++)
	{
		...
		$cur_item = str_replace('{GSSE_INCL_EOO}',$eoo,$cur_item);
		$cur_file = $link;
		$cur_file = str_replace('{GSSE_INCL_LINKCLASS}','',$cur_file);
		/*$cur_file = str_replace('{GSSE_INCL_LINKURL}','inc/freedownload.inc.php?cfile=' . $aDownloads[$ad]['filename'],$cur_file);*/
		$cur_file = str_replace('{GSSE_INCL_LINKURL}','javascript:freedownload(\'' . $aDownloads[$ad]['filename'] . '\');', $cur_file);
		$cur_file = str_replace('{GSSE_INCL_LINKTARGET}','',$cur_file);
		$cur_file = str_replace('{GSSE_INCL_LINKNAME}',$aDownloads[$ad]['filename'],$cur_file);
		$cur_item = str_replace('{GSSE_INCL_DATATITLE}',$cur_file,$cur_item);
		$cur_item = str_replace('{GSSE_INCL_DATAVALUE}',$aDownloads[$ad]['title'],$cur_item);
		$all_items .= $cur_item;
	}
}
...
if($all_items == ''){
    ...
    $detailsslider = str_replace('{GSSE_CLASS_DOWNLOADS}','',$detailsslider);
}
/*HTML-text 1*/
...
if($_SESSION['aitem']['itemHtmlText1'] == ''){
    ...
    $detailsslider = str_replace('{GSSE_CLASS_HTMLTEXT1}','',$detailsslider);
}
/*HTML-Text 2*/
...
if($_SESSION['aitem']['itemHtmlText2'] == ''){
    ...
    $detailsslider = str_replace('{GSSE_CLASS_HTMLTEXT2}','',$detailsslider);
}
